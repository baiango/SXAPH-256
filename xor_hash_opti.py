...
def chi2_benchmark(bucket_sizes=[i for i in range(1, 101) if is_prime(i)], hash_list=load_test('BruteX_password.list')):
	mul_hash_score = test_hash_distribution(mul_hash, bucket_sizes=bucket_sizes, u64x4_chunks=hash_list)
	print("mul_hash chi2_benchmark score:", mul_hash_score)
	shift_xor_add_hash_score = test_hash_distribution(shift_xor_add_impl, bucket_sizes=bucket_sizes, u64x4_chunks=hash_list)
	print("shift_xor_add_hash chi2_benchmark score:", shift_xor_add_hash_score)
	crc32_hash_score = test_hash_distribution(crc32_hash, bucket_sizes=bucket_sizes, u64x4_chunks=hash_list)
	print("crc32_hash chi2_benchmark score:", crc32_hash_score)
	sha256_hash_score = test_hash_distribution(sha256_hash, bucket_sizes=bucket_sizes, u64x4_chunks=hash_list)
